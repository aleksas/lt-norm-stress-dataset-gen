...
from static.get_zodis import get_all_names
...
def convert(d):
    ...
    if isinstance(d, dict):
        ...
        if len(d) > 0 and isinstance(d[0], str):
            ...
            for s in d[1:]:
                s = pe.process_and_collapse(s0 + s, word_format='ascii_stressed_word').lower() if s else s
                res.append(s)
...
data = convert(get_all_names())

with open("raw/zodziai.stressed.json", "w", encoding="utf-8") as fp:
